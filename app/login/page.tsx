...
export default function LoginPage() {
  ...
  const { signIn, signUp, isAuthenticated, loading: authLoading, user, session } = useAuth()
  ...
  useEffect(() => {
    if (isAuthenticated && !authLoading) {
      console.log('Login successful detected, redirecting...')
      setRedirecting(true)
      router.push("/dashboard")
    }
      ...
      if (result.error) {
        ...
        setTimeout(() => {
          if (isAuthenticated) {
            ...
            console.log('Auth state not updated yet, waiting...')
        ...
        >
          ...
          {process.env.NODE_ENV === 'development' && (
            <p className="text-xs text-muted-foreground mt-2">
              <Link href="/test-supabase" className="text-primary hover:underline">
                🔧 Test Authentication
              </Link>
            </p>
          )}

-- Complete Database Setup for AI Interview Assistant
-- This script creates all necessary tables with the correct schema
...
-- Resume analysis results (complete schema)
...
DROP TRIGGER IF EXISTS update_job_analysis_updated_at ON public.job_analysis;
CREATE TRIGGER update_job_analysis_updated_at BEFORE UPDATE ON public.job_analysis
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP POLICY IF EXISTS "Users can view own job analysis" ON public.job_analysis;
